...
if __name__ == "__main__":
    ...
    for i in range(len(dataset)):
        ...
        x = x.cuda()
        print(i, len(dataset))
        prediction, pABar, radius = smoothed_classifier.certify(x, args.N0, args.N, args.alpha, args.batch)
        after_time = time()
        correct = int(prediction == label)

        time_elapsed = str(datetime.timedelta(seconds=(after_time - before_time)))
        print("{}/{}\t{}\t{}\t{}\t{}\t{}\t{}".format(
            i, len(dataset), label, prediction, pABar, radius, correct, time_elapsed), file=f, flush=True)
